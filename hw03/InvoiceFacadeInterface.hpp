...
class InvoiceFacadeInterface {
 public:
  ...
  virtual unsigned createInvoice(unsigned supplierId,
                                 unsigned customerId,
                                 unsigned variableSymbol,
                                 unsigned constantSymbol,
                                 std::string issuedDate,
                                 std::string dueDate,
                                 std::vector<InvoiceItem> items) = 0;

  virtual std::string getInvoiceSupplierName(unsigned invoiceId) const = 0;
  virtual std::string getInvoiceSupplierRegistrationId(
      unsigned invoiceId) const = 0;
  ...
  virtual std::string getInvoiceSupplierBankAccount(
      unsigned invoiceId) const = 0;

  virtual std::string getInvoiceCustomerName(unsigned invoiceId) const = 0;
  virtual std::string getInvoiceCustomerRegistrationId(
      unsigned invoiceId) const = 0;
  ...
  virtual const std::vector<InvoiceItem>& getItems(
      unsigned invoiceId) const = 0;
