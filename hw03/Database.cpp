...
std::vector<Invoice const*> Database::getInvoicesByCustomer(
    unsigned int customerId) {
  std::vector<Invoice const*> invoicesByCustomer;
  for (const std::unique_ptr<Invoice>& invoice : m_invoices) {
    if (invoice->m_buyer.getId() == customerId)
      invoicesByCustomer.push_back(invoice.get());
  }
  return invoicesByCustomer;
}

std::vector<Invoice const*> Database::getInvoicesBySupplier(
    unsigned int supplierId) {
  std::vector<Invoice const*> invoicesBySupplier;
  for (const std::unique_ptr<Invoice>& invoice : m_invoices) {
    if (invoice->m_supplier.getId() == supplierId)
      invoicesBySupplier.push_back(invoice.get());
  }
