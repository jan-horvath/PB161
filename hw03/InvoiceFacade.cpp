...
unsigned int InvoiceFacade::createInvoice(unsigned supplierId,
                                          unsigned customerId,
  ...
  assert(database.addInvoice(Invoice(++counter, *supplier, *customer,
                                     variableSymbol, constantSymbol, issuedDate,
                                     dueDate, items)));
  return counter;
}
...
std::string InvoiceFacade::getInvoiceSupplierName(unsigned invoiceId) const {
  ...
  if (invoiceFound == nullptr)
    return "";
  return invoiceFound->m_supplier.getName();
}

std::string InvoiceFacade::getInvoiceSupplierRegistrationId(
    unsigned invoiceId) const {
  auto invoiceFound = database.getInvoice(invoiceId);
  if (invoiceFound == nullptr)
    return "";
...
std::string InvoiceFacade::getInvoiceSupplierBankAccount(
    unsigned invoiceId) const {
  auto invoiceFound = database.getInvoice(invoiceId);
  if (invoiceFound == nullptr)
    return "";
...
std::string InvoiceFacade::getInvoiceCustomerRegistrationId(
    unsigned invoiceId) const {
  auto invoiceFound = database.getInvoice(invoiceId);
  if (invoiceFound == nullptr)
    return "";
...
unsigned int InvoiceFacade::getTotalInvoicedToCustomer(
    unsigned customerId) const {
  if (!database.getSubject(customerId))
    return UINT32_MAX;  // customer not found

  unsigned int total = 0;
  for (auto invoice : database.getInvoicesByCustomer(customerId)) {
    for (auto item : invoice->m_items) {
      total +=
          item.getAmount() + ((item.getAmount() * item.getTaxRate()) / 100);
    }
  }
  return total;
}

unsigned int InvoiceFacade::getTotalInvoicedFromSupplier(
    unsigned supplierId) const {
  if (!database.getSubject(supplierId))
    return UINT32_MAX;  // customer not found
...
const std::vector<InvoiceItem>& InvoiceFacade::getItems(
    unsigned invoiceId) const {
