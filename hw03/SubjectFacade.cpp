...
unsigned int SubjectFacade::createSubject(std::string name,
                                          std::string registrationId,
                                          std::string vatId,
                                          std::string address,
                                          std::string bankAccount) {
  assert(database.addSubject(
      Subject(++counter, name, registrationId, vatId, address, bankAccount)));
  return counter;
}

void SubjectFacade::updateSubject(unsigned subjectId,
                                  std::string name,
                                  std::string registrationId,
                                  std::string vatId,
                                  std::string address,
  ...
  updateSuccessful = database.updateSubject(
      Subject(subjectId, name, registrationId, vatId, address, bankAccount));
}
