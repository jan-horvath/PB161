...
bool UpdateSubject(unsigned id, SubjectsFacadeInterface &subjectFacade) {
    if (subjectFacade.getSubjectName(id).empty()) return false;
    string name, regId, vatId, address, bankAcc;
    cout << "subject name: ";
    getline(cin, name);
    cout << "registration id: ";
...
bool PrintSubject(unsigned id, SubjectsFacadeInterface &subjectFacade) {
    ...
    cout << "registration id: " << subjectFacade.getSubjectRegistrationId(id) << endl;
...
unsigned CreateItems(vector<InvoiceItem> &items, unsigned count) {
    ...
    for (unsigned i = 1; i <= count; ++i) {
        ...
        try {itemAmount = stoi(toBeConverted);}
        catch (invalid_argument&) {return 3;}

        cout << "item " << i << " tax rate: ";
        getline(cin, toBeConverted);
        try {itemTaxRate = stoi(toBeConverted);}
        catch (invalid_argument&) {return 3;}
        if (itemTaxRate > 100) {return 4;}
...
unsigned CreateInvoice(InvoiceFacadeInterface &invoiceFacade, SubjectsFacadeInterface &subjectFacade) {
    ...
    if (!subjectFacade.containsSubject(supplierId)) return 1;

    cout << "customer id: ";
    getline(cin, toBeConverted);
    customerId = stoi(toBeConverted);
    if (!subjectFacade.containsSubject(customerId)) return 2;

    cout << "variable symbol: ";
    getline(cin, toBeConverted);
    try {varSymbol = stoi(toBeConverted);}
    catch (invalid_argument&) {return 3;}

    cout << "constant symbol: ";
    getline(cin, toBeConverted);
    try {constSymbol = stoi(toBeConverted);}
    catch (invalid_argument&) {return 3;}
    ...
    cout << "add items: ";
    getline(cin, toBeConverted);
    try {numOfItems = stoi(toBeConverted);}
    catch (invalid_argument&) {return 3;}

    unsigned error = CreateItems(items, numOfItems);
    if (error != 0) return error;

    unsigned id = invoiceFacade.createInvoice(supplierId, customerId, varSymbol, constSymbol, issuedDate, dueDate,
                                              items);
...
bool PrintInvoice(unsigned id, InvoiceFacadeInterface &invoiceFacade) {
    if (invoiceFacade.getInvoiceCustomerName(id).empty()) return false;
    cout << "# Customer" << endl;
    cout << "subject name: " << invoiceFacade.getInvoiceCustomerName(id) << endl;
    cout << "registration id: " << invoiceFacade.getInvoiceCustomerRegistrationId(id) << endl;
    cout << "vat id: " << invoiceFacade.getInvoiceCustomerVatId(id) << endl;
    cout << "address: " << invoiceFacade.getInvoiceCustomerAddress(id) << endl << endl;
    cout << "# Supplier" << endl;
    cout << "subject name: " << invoiceFacade.getInvoiceSupplierName(id) << endl;
    cout << "registration id: " << invoiceFacade.getInvoiceSupplierRegistrationId(id) << endl;
    cout << "vat id: " << invoiceFacade.getInvoiceSupplierVatId(id) << endl;
    cout << "address: " << invoiceFacade.getInvoiceSupplierAddress(id) << endl;
    cout << "bank account: " << invoiceFacade.getInvoiceSupplierBankAccount(id) << endl << endl;
    for (InvoiceItem item : invoiceFacade.getItems(id)) {
        cout << "* " << item.getTitle() << ", " << item.getAmount() << ", " << item.getTaxRate() << "%" << endl;
    }
...
void init() {
    ...
    while (choice != "quit") {
        ...
        if (choice == "createInvoice") {
            ...
            if (error == 1) cerr << "supplier not found" << endl;
            if (error == 2) cerr << "customer not found" << endl;
            if (error == 3) cerr << "not a number" << endl;
            if (error == 4) cerr << "invalid tax rate" << endl;
        }
        ...
        if (unknownCommand) {cerr << "unknown command " << choice << endl;}
        cout << "> ";
        getline(cin, choice);
        unknownCommand = true;
    }
