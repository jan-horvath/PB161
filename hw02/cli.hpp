...
struct Option {
  ...
  // ADD ACCUMULATE
  Option(char shortName, const std::string &longName, const std::string &help,
         OptionType ty = OT_String)
      : _longName(longName), _shortName(shortName), _help(help), _type(ty),
        _accum(A_AssertSingle), _needsParameter(true) {}
...
struct Parser {
  ...
  Option *add(char shortName, const std::string &longName,
              const std::string &help, OptionType ty = OT_String);
...
struct FileCommandsParser {
  ...
  const std::map<const std::string, std::unique_ptr<Parser>> &getSubcommands() {
    return _subcommands;
  }
